...
"""
import os
#os.chdir("D:\\Karola\\birdsong_model_conf_imm_lm\\half_deviation_from_the_pop\\posizetest_4000cycles\\raw_simulation_data\\")
os.chdir("C:\\Users\\bartakarola\\OneDrive - elte.hu\\birdsong_model_conf_imm_lm\\immigrants_with_non_random_repertoires\\half_deviation_from_the_pop\\popsizetest_withlowdiversity\\raw_simulation_data\\")
...
def song_learning(num_birds, ism):
    #print(a, migr, lm, ism)
    a = 1.1
    migr = 0.03
    lm = 0.1
    ...
    subset_length = 10
    syllables_subset = sample(syllables, subset_length)
    ...
    # the learning rule will not work otherwise
        birds0[bi].syll_rep=sample(syllables_subset, subset_length)*round(indprodveclen/subset_length)
    ...
    pop10_rep.to_csv('pop10rep_'+learnmode+'_'+str(num_cycle)+'x'+str(learningoccasions)+'sys'+str(num_syll)+str(migr)[2:]+str(lm)[2:]+str(ism)[0]+'_popsize'+str(num_birds)+'.csv')
    pop10_indrep.to_csv('pop10_indrep_'+learnmode+'_'+str(num_cycle)+'x'+str(learningoccasions)+'sys'+str(num_syll)+str(migr)[2:]+str(lm)[2:]+str(ism)[0]+'_popsize'+str(num_birds)+'.csv')

    #younginds_repchange.to_csv('younginds_repchange'+learnmode+'_'+str(num_cycle
...
params = {
        'num_birds' : [num_birds4],
        'ism' : [x for x in range(num_simrep)]
        }

num_comb = len(params['num_birds']) * len(params['ism'])

def main():
    keys = list(params)
    param_combs = pd.DataFrame(columns = ['num_birds', 'ism'], index = [x for x in range(num_comb)])
    rownum = 0
