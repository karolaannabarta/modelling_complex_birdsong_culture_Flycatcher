...
def song_learning(num_syll, ism):
    #print(a, migr, lm, ism)
    a = 1.1
    migr = 0.03
    lm = 0.1
    ...
    pop10_rep.to_csv('pop10rep_'+learnmode+'_'+str(num_cycle)+'x'+str(learningoccasions)+'sys'+str(num_syll)+str(migr)[2:]+str(lm)[2:]+str(ism)[0]+'.csv')
    pop10_indrep.to_csv('pop10_indrep_'+learnmode+'_'+str(num_cycle)+'x'+str(learningoccasions)+'sys'+str(num_syll)+str(migr)[2:]+str(lm)[2:]+str(ism)[0]+'.csv')
...
params = {
        'num_syll' : [num_syll1, num_syll2, num_syll3],
        'ism' : [x for x in range(num_simrep)]
        }

num_comb = len(params['num_syll']) * len(params['ism'])

def main():
    keys = list(params)
    param_combs = pd.DataFrame(columns = ['num_syll', 'ism'], index = [x for x in range(num_comb)])
    rownum = 0
