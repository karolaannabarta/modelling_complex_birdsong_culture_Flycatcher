...
"""
import os
#os.chdir("D:\\Karola\\birdsong_model_conf_imm_lm\\half_deviation_from_the_pop\\numsylltest_4000cycles\\raw_simulation_data\\")

os.chdir("C:\\Users\\karol\\OneDrive - elte.hu\\birdsong_model_conf_imm_lm\\immigrants_with_non_random_repertoires\\half_deviation_from_the_pop\\learnocctest_4000cycles\\raw_simulation_data\\")
...
def song_learning(learningoccasions, ism):
    #print(a, migr, lm, ism)
    num_syll = 2000
    a = 1.1
    migr = 0.03
    ...
    pop10_rep.to_csv('pop10rep_'+learnmode+'_'+str(num_cycle)+'xlearnocc'+str(learningoccasions)+'sys'+str(num_syll)+str(migr)[2:]+str(lm)[2:]+str(ism)[0]+'.csv')
    pop10_indrep.to_csv('pop10_indrep_'+learnmode+'_'+str(num_cycle)+'xlearnocc'+str(learningoccasions)+'sys'+str(num_syll)+str(migr)[2:]+str(lm)[2:]+str(ism)[0]+'.csv')
...
params = {
        'learningoccasions' : [learn_occ1, learn_occ2, learn_occ3],
        'ism' : [x for x in range(num_simrep)]
        }

num_comb = len(params['learningoccasions']) * len(params['ism'])

def main():
    keys = list(params)
    param_combs = pd.DataFrame(columns = ['learningoccasions', 'ism'], index = [x for x in range(num_comb)])
    rownum = 0
